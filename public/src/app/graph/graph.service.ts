...
export class GraphService {
	...
	constructor(private http: HttpClient) { }
        ...
        deepfirstSearch(vertice: string, metodo: string, graph: graphFromBack){
			return this.http.post<graphResponse>(environment.url + 'exec/deepfirst_search/'+ metodo  + '/' + vertice , graph);
        }
        
        breadthfirstSearch(vertice: string, metodo: string, graph: graphFromBack){
			return this.http.post<graphResponse>(environment.url + 'exec/breadthfirst_search/'+ metodo  + '/' + vertice , graph);
        }
        
        defineDistances(vertice: string, metodo: string, graph: graphFromBack){
			return this.http.post<graphResponse>(environment.url + 'exec/define_distances/'+ metodo  + '/' + vertice , graph);
        }
